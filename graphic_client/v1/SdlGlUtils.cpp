//
// sdlglutils.cpp for nibbler in /home/manu/tek2/C++/nibbler/v1/opengl/plateau/v3
//
// Made by Manu
...
namespace		graphic
{
  GLuint		SdlGlUtils::loadTexture(const char * filename,
						bool useMipMap)
  {
    GLuint glID;
    ...
    gl_surface = SDL_ConvertSurface(picture_surface,&format,SDL_HWSURFACE);
  ...
  int			SdlGlUtils::takeScreenshot(const char * filename)
  {
    GLint viewport[4];
    Uint32 rmask, gmask, bmask, amask;
    ...
    picture = SDL_CreateRGBSurface(SDL_HWSURFACE,viewport[2],viewport[3], 32,
  ...
  SDL_Surface *			SdlGlUtils::flipSurface(SDL_Surface * surface)
  {
    int current_line,pitch;
    SDL_Surface * fliped_surface = SDL_CreateRGBSurface(SDL_HWSURFACE,
							surface->w,surface->h,
  ...
  void			SdlGlUtils::drawAxis(double scale)
  {
  ...
  int		SdlGlUtils::initFullScreen(unsigned int * width,
					   unsigned int * height)
  {
    SDL_Rect ** modes;
  ...
  int			SdlGlUtils::XPMFromImage(const char * imagefile,
						 const char * XPMfile)
  {
    ...
    image32bits = SDL_CreateRGBSurface(SDL_HWSURFACE,
				       image->w,image->h,
  ...
  SDL_Cursor *			SdlGlUtils::cursorFromXPM(const char * xpm[])
  {
    int i, row, col;
    int width, height;
    Uint8 * data;
